...
{
    public class ImgAiVectorService
    {
        ...
        public async Task InitModelsAsync(string modelsPath)
        {
            try
            {
                if (!File.Exists(Path.Combine(modelsPath, "Vit-L-14.img.fp32.onnx")))
                {
                    throw new FileNotFoundException($"Vit图片模型文件不存在,请下载放置并命名:{Path.Combine(modelsPath, "Vit-L-14.img.fp32.onnx")}", Path.Combine(modelsPath, "Vit-L-14.img.fp32.onnx"));
                }
                if (!File.Exists(Path.Combine(modelsPath, "Vit-L-14.txt.fp32.onnx")))
                {
                    throw new FileNotFoundException($"Vit文本模型文件不存在,请下载放置并命名:{Path.Combine(modelsPath, "Vit-L-14.txt.fp32.onnx")}", Path.Combine(modelsPath, "Vit-L-14.txt.fp32.onnx"));
                }
                if (!File.Exists(Path.Combine(modelsPath, "vocab.txt")))
                {
                    throw new FileNotFoundException($"词汇表文件不存在,请下载放置并命名:{Path.Combine(modelsPath, "vocab.txt")}", Path.Combine(modelsPath, "vocab.txt"));
                }
