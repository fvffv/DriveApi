...
namespace drive_api.Services.AdminManagement
{
    ...
    public class AdminService(Helper helper, ILogger<AdminService> logger, ICache icache, ISqlSugarClient sqlSugarClient, AppConfigInfo appConfigInfo, FileService fh)
    {
        ...
        private static int? GetSqliteLogLevel(string? level)
        {
        ...
        private static string? GetSqliteLogEvent(string? properties)
        {
            ...
            try
            {
                ...
                {
                    Properties = document.RootElement
                });
            }
            catch (JsonException)
            {
        ...
        private static SysLogs ToSysLogs(SqliteSysLogs log)
        {
            return new SysLogs
            {
                Message = log.Message,
                Level = GetSqliteLogLevel(log.Level),
                Timestamp = log.Timestamp,
                Exception = log.Exception,
                LogEvent = GetSqliteLogEvent(log.LogEvent)
            };
        }
        ...
        public async Task<DefaultMsg> GetDataStatistics()
        {
            ...
            if (_sqlSugarClient.DbMaintenance.IsAnyTable("sys_logs"))
            {
                ...
                {
                    log5 = _sqlSugarClient.Queryable<SqliteSysLogs>()
                    .OrderByDescending(x => x.Timestamp)
                    .Take(5).ToList()
                    .Select(x => new
                    {
                        Level = GetSqliteLogLevel(x.Level),
                        x.Message,
                        x.Timestamp
                    })
                    .ToList();
        ...
        public async Task<DefaultMsg> GetLogs(int pageIndex)
        {
            ...
            if (_sqlSugarClient.CurrentConnectionConfig.DbType == DbType.Sqlite)
            {
                var info = (await _sqlSugarClient.Queryable<SqliteSysLogs>().OrderBy(x => x.Timestamp, OrderByType.Desc).ToPageListAsync(pageIndex, 20, totalCount))
                    .Select(ToSysLogs)
                    .ToList();
                return new DefaultMsg(0, "success", new
                {
            ...
            var postgresInfo = await _sqlSugarClient.Queryable<SysLogs>().OrderBy(x => x.Timestamp, OrderByType.Desc).ToPageListAsync(pageIndex, 20, totalCount);
            return new DefaultMsg(0, "success", new
            {
                Log = postgresInfo,
                TotalCount = totalCount.Value
