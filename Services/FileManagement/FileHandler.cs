...
namespace drive_api.Services.FileManagement
{
    ...
    {
        private readonly ILogger<FileHandler> _logger = logger;
        ...
        public string GetFileStorageDirectory(long fileBytes)
        {
            ...
            // 标准化：在抛出异常阻断流程前，记录一条 Error 级别的日志，便于统一的日志面板（如 ELK）捕获并触发严重告警
        ...
        {
            var paths = appConfigInfo.fileSetting.LocalFilePath?.Split('|') ?? new string[] { defaultfileDirectory };
            ...
            int pageIndex = 1, int pageSize = 50)
        {
            ...
            try
            {
                ...
                userFilesInfo.FileInfos = (await _sqlSugarClient.Queryable<fi>()
                    ...
                    .OrderBy(x => x.CreationTime, OrderByType.Desc) // 建议加上排序保证分页稳定
        ...
        private async Task<fi> CheckForDuplicateAsync(string fileHash)
        {
            return await _sqlSugarClient.Queryable<fi>()
                .FirstAsync(x => x.FileHash == fileHash && x.IsDeleted == false);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<DefaultMsg> SaveToAsync(string userId, string folderId = defaultGuid, string fileId = "",
            string hash256 = "")
        {
            ...
            // 1. 策略一：通过 FileId (分享ID) 查找
            if (!string.IsNullOrWhiteSpace(fileId) && Guid.TryParse(fileId, out Guid guidFileId))
            {
                // 标准化：消除 $ 插值，提取 UserId 和 FileId 作为独立检索字段
                _logger.LogInformation("用户尝试转存文件. UserId: {UserId}, SourceFileId: {FileId}", userId, fileId);
                //sourceItem = await _sqlSugarClient.Queryable<fi>()
                //    .FirstAsync(x => x.Id == guidFileId && x.FileShare != null && x.FileShare != "" && !x.IsDeleted);
            }
            // 2. 策略二：通过 Hash (秒传) 查找
            else if (!string.IsNullOrWhiteSpace(hash256))
            {
                // 标准化：消除 $ 插值
                _logger.LogInformation("用户通过hash上传(秒传)文件. UserId: {UserId}, FileHash: {FileHash}", userId, hash256);
                sourceItem = await _sqlSugarClient.Queryable<fi>()
                    .FirstAsync(x => x.FileHash == hash256 && x.IsDeleted == false);
            }

            // 3. 如果没找到源文件，直接返回失败
            if (sourceItem == null)
            {
                // 标准化：业务失败路径升级为 LogWarning，方便错误追踪
                _logger.LogWarning(
            ...
            _logger.LogInformation(
                ...
                userId, newItem.Id, fileId, hash256, newItem.FileName);
            ...
            else
            {
                ...
                if (type.Any())
                {
                    ...
                    if (mimeType.StartsWith("image/"))
                    {
                        ...
                        { FileId = newItem.Id, StoragePath = newItem.StoragePath, UserId = newItem.UserId };
                        //缩略图
        ...
        private async Task HandlePhysicalFilesDeletionAsync(List<FileDeletionItem> itemsToDelete, Guid uid)
        {
            ...
            foreach (var item in itemsToDelete)
            {
                ...
                if (!isReferenced)
                {
                    ...
                    {
                        _logger.LogError(ex, "用户{UserId}物理删除文件 '{StoragePath}' 失败：{ErrorMessage}", uid,
...
";
        ...
        {
            if (!Guid.TryParse(userId, out Guid uid)) return new DefaultMsg(1, "uid格式错误", null);
            if (string.IsNullOrEmpty(searchInfo.Keyword) && searchInfo.FileType?.Length == 0 && searchInfo.EndCreationTime == null &&
            ...
            if (isAI)
            {
                ISugarQueryable<fi> query = _sqlSugarClient.Queryable<fi>()
                        .Where(x => x.UserId == uid && x.FileName.Contains(searchInfo.Keyword) && x.IsDeleted == false);
                //判断文件类型
                ...
                userFilesInfo.FileInfos = await query.Select(x => new UserFilesInfoItem()
                {
                    ...
                    .ToArrayAsync();
                //搜索文件夹
                if ((searchInfo.FileType == null || searchInfo.FileType?.Length == 0) || (searchInfo.FileType?.Length != 0 && !string.IsNullOrEmpty(searchInfo.FileType.FirstOrDefault(x => x == "文件夹"))))
                {
                    ISugarQueryable<UserFolderInfo> queryFolder = _sqlSugarClient.Queryable<UserFolderInfo>()
                        .Where(x => x.UserId == uid && x.FolderName.Contains(searchInfo.Keyword) && x.IsDeleted == false);
            ...
            //计算文本向量
            if (_appConfigInfo.aiSetting.Enable && isAI)
            {

                //判断结果是否包含图片类型，如果包含则进行向量搜索
                if ((searchInfo.FileType == null || searchInfo.FileType?.Length == 0) || searchInfo.FileType?.Length != 0 && !string.IsNullOrEmpty(searchInfo.FileType.FirstOrDefault(x => x == "图片")))
                {
                    ...
                    WHERE user_id = @userId 
                      AND (1 - (vector <=> @vector::vector)) > {_appConfigInfo.aiSetting.ImageCosineThreshold}
                    ORDER BY vector <=> @vector::vector; 
                ";
                    //注入参数
                    List<Guid> fileIds = await _sqlSugarClient.Ado.SqlQueryAsync<Guid>(sql, new
                    {
                        vector = textVector,
                        userId = uid
                    });
        ...
        public async Task<DefaultMsg> GetFullFolderPathAsync(string userId, string folderId)
        {
            ...
            //拼接最终的 SQL
            string sql = $@"
    {cteKeyword} FolderPath AS (
            SELECT id, parent_id, folder_name, 1 AS level
            FROM folder_info
            WHERE id = @fid AND user_id = @uid  

            UNION ALL

            SELECT f.id, f.parent_id, f.folder_name, p.level + 1
            FROM folder_info f
            INNER JOIN FolderPath p ON f.id = p.parent_id
            WHERE f.user_id = @uid AND f.id <> p.id
        )
        SELECT folder_name FROM FolderPath ORDER BY level DESC;
    ";

            try
            {
                var folderNames = await _sqlSugarClient.Ado.SqlQueryAsync<string>(sql, new { fid, uid });
                if (folderNames.Count() == 0) return new DefaultMsg(1, "文件夹不存在", null);

                var cleanNames = folderNames?
                    .Where(name => !string.IsNullOrWhiteSpace(name) && name.Trim() != "/")
                    .ToList() ?? new List<string>();
                string fullPath = cleanNames.Any()
                    ? "/" + string.Join("/", cleanNames)
                    : "/";

                // 写入缓存 (可以设置一个较长的过期时间，比如 24 小时)
